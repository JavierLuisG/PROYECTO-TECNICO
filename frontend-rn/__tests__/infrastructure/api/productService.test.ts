...
type MockAxiosInstance = { get: jest.Mock; post: jest.Mock; put: jest.Mock; delete: jest.Mock };
...
describe('productService.getAll', () => {
  it('returns normalized products array', async () => {
...
describe('productService.getById', () => {
  ...
  it('normalizes ISO timestamp dates to YYYY-MM-DD', async () => {
    api.get.mockResolvedValue({
      data: { ...mockProduct, date_release: '2023-02-01T00:00:00.000Z', date_revision: '2024-02-01T00:00:00.000Z' },
    });
    const result = await productService.getById('trj-crd');
    expect(result.date_release).toBe('2023-02-01');
    expect(result.date_revision).toBe('2024-02-01');
  });
